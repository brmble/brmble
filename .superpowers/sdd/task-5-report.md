...
# Collaborative Paint Task 5 Report
...
- Added canonical paint web types with string session, stroke, source-event, and Matrix identifiers.
- Added the `paintApi` mutation surface for the Task 4 bridge events. Stroke mutations retain only normalized stroke input and do not send author identity. Snapshot requests use the `paint.request` / `paint.response` correlation and timeout pattern.
- Added deterministic canvas helpers for clamped pointer normalization, intrinsic-resolution stroke rendering, annotation-only erasing through `destination-out`, authenticated Matrix media loading, canvas sizing, and PNG composition.
- Added `usePaintSession(sessionId)` reconciliation for canonical broadcasts: sequence sorting, server-ID deduplication, preview replacement by author/correlation key, stale-generation filtering, and snapshot recovery on permanent revision gaps.

## TDD Evidence

1. Added `paintCanvas.test.ts` and `usePaintSession.test.tsx` before their production modules existed.
2. Ran `npm.cmd run test -- src/utils/paintCanvas.test.ts src/hooks/usePaintSession.test.tsx` before implementation.
3. The run failed as expected with Vite import-resolution errors for the missing `./paintCanvas` and `../api/paint` modules.
4. Implemented the smallest canvas/API/hook surfaces needed for the tests. The first green run caught a revision-gap timing issue; the hook now evaluates the gap from a current snapshot ref and requests recovery synchronously.

## Verification

- `npm.cmd run test -- src/utils/paintCanvas.test.ts src/hooks/usePaintSession.test.tsx`: PASS, 2 files and 5 tests passed.
- `npm.cmd run type-check`: PASS.
- `git diff --check`: PASS.

## Files Changed

- `src/Brmble.Web/src/api/paint.ts`
- `src/Brmble.Web/src/types/paint.ts`
- `src/Brmble.Web/src/utils/paintCanvas.ts`
- `src/Brmble.Web/src/utils/paintCanvas.test.ts`
- `src/Brmble.Web/src/hooks/usePaintSession.ts`
- `src/Brmble.Web/src/hooks/usePaintSession.test.tsx`
- `.superpowers/sdd/task-5-report.md`
