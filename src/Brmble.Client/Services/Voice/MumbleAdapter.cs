...
internal sealed class MumbleAdapter : BasicMumbleProtocol, VoiceService
{
    ...
    /// Returns null when the payload carries no companion at all, so callers can
    /// distinguish "not announced" from "reset to default".
    /// </summary>
    internal static string? ParseWireCompanionIdOrNull(System.Text.Json.JsonElement element)
    {
    ...
    public MumbleAdapter(NativeBridge bridge, IntPtr hwnd, CertificateService? certService = null, IAppConfigService? appConfigService = null, VoiceIdleTracker? voiceIdleTracker = null)
    {
        ...
        _inputRouter.ShortcutReleased += (action, forced) => {
            ...
            // tearing down (Disconnect/channel transition/Suspend) or the
            // binding got evicted under a held button. Otherwise
            // disconnecting while toggleMute is held would toggle mute as
