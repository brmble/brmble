...
export function PaintToolbar({ tool, color, width, onTool, onColor, onWidth }: Props) {
  return <div className="paint-toolbar" aria-label="Paint tools">
    ...
    <div className="paint-swatches">{PAINT_COLORS.map(({ id, label, value }) => <button key={value} aria-label={`Color ${label}`} className={['paint-swatch', `paint-swatch-${id}`, color === value ? 'active' : ''].filter(Boolean).join(' ')} onClick={() => onColor(value)} />)}</div>
