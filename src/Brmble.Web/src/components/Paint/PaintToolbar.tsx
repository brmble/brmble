...
export function PaintToolbar({ tool, color, width, onTool, onColor, onWidth }: Props) {
  return <div className="paint-toolbar" aria-label="Paint tools">
    {(['pen', 'line', 'eraser'] as const).map(value => <button key={value} type="button" className={tool === value ? 'active' : ''} onClick={() => onTool(value)}>{value === 'pen' ? 'Pen' : value === 'line' ? 'Line' : 'Eraser'}</button>)}
