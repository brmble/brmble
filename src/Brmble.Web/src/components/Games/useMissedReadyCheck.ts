...
export interface MissedReadyCheck {
   ...
   *
   * Missing your own check outranks the opponent missing theirs, so this is what decides
   * between the two notification forms: only a readied local player can be shown the
   * "your opponent didn't ready" copy. When neither readied, the opponent still appears
   * in `unreadyOpponents`, so that array alone cannot make the call.
   */
