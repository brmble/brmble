        ...
        private void EncodingThreadEntry(out Exception exception)
        {
            ...
            try
            {
                while (IsEncodingThreadRunning)
                {
                    ...
                    {
                        var encoded = encodedTargettedSpeech.Value.EncodedPcm;
                        ...
                        // Voice header: [type|target, sequence...]
                        byte[] voiceHeader = new byte[1 + sequence.Length];
                        voiceHeader[0] = typeTarget;
                        sequence.CopyTo(voiceHeader, 1);

                        // Opus: one frame per packet, size varint (no splitting)
                        byte[] header = Var64.writeVarint64_alternative((UInt64)encoded.Length);
                        byte[] packedData = new byte[voiceHeader.Length + header.Length + encoded.Length];

                        Array.Copy(voiceHeader, 0, packedData, 0, voiceHeader.Length);
                        Array.Copy(header, 0, packedData, voiceHeader.Length, header.Length);
                        Array.Copy(encoded, 0, packedData, voiceHeader.Length + header.Length, encoded.Length);
